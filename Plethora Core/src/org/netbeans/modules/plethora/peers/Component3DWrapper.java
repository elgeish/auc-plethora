 ...
 * @version 1.00
 */
public class Component3DWrapper extends Component3D implements ShapeWrapper {
    ...
    public void setIdentifier(String id) {
        try {
            if (!lg3dDataDocument.isUnique(id)) {
                DialogDisplayer.getDefault().notify(new NotifyDescriptor.Message(
                        Bundle.getBundleString("MSG_IdAlreadyExists"),
                        NotifyDescriptor.INFORMATION_MESSAGE));
            } else {
                
                for(Node.Property property : getShapePeer().getEventsSet().getProperties()) {
                    if (property.getValue() == null || property.getValue().equals("")) {
                        property.setValue("initialEditValue", id + property.getName());
                    } else if (property.getValue().equals(getName() + property.getName())) {
                        property.setValue(id + property.getName());
                        property.setValue("initialEditValue", id + property.getName());
                    }
                }
                
                lg3dDataDocument.renameElement(getName(), id);
                setName(id);
            }
    ...
    public void setPreferredSize(Vector3f vector) {
        if (vector.x == 0 && vector.y == 0 && vector.z == 0) {
            ...
            setProperty(PROP_PREFERRED_SIZE, Vector3f.class,
                    new SimpleParameter(float.class.getName(), vector.x + "f"),
                    new SimpleParameter(float.class.getName(), vector.y + "f"),
                    new SimpleParameter(float.class.getName(), vector.z + "f"));
        }
